...
import {
  ...
  PLAN_PAGE,
  PROGRESS_STEPS,
} from "../data/planOptions.js";

function ClarifyComposer({ message, onChange, onSend }) {
  return html`
    <div className="rounded-2xl bg-white p-0.5 shadow-md ring-1 ring-[#DED1F8]">
      ...
      <div className="flex items-center justify-between rounded-b-2xl border-t border-[#ECE5FB] px-4 py-2.5">
        <img
          src="./assets/icon-upload-clip.svg"
          alt=""
          className="h-4 w-[17px] select-none"
          draggable=${false}
        />
        <button
          type="button"
        ...
        >
          <img
            src="./assets/icon-search-field-arrow.svg"
            alt=""
            className="h-[13px] w-[17px] select-none"
            draggable=${false}
          />
        </button>
      </div>
...
function ConfirmComposer({ onSend }) {
  return html`
...
export function Sidebar({
  ...
  const confirmText =
    "Great! We have established the basics to your Summer Plan. Let’s confirm these are correct.";
  const keyPiecesTitle = "Thank you!";
  const keyPiecesSub =
    "Our experts are building out the key pieces to solidify your plan.";
  const thePlanReady = "You're plan is ready!";
  const thePlanSub = "Have changes? Let us know what you need.";
  const reviseText =
    "“Let’s keep working on these”. Please provide any additions or revisions to the basics.";

  return html`
    <aside
      className="flex h-full w-[30%] min-w-[300px] max-w-xl flex-col border-r-2 border-[#D7C2FF] bg-[#EADFFF] px-6 py-8"
    >
      <div className="flex min-h-0 flex-1 flex-col gap-7">
        <header className="shrink-0 space-y-4">
          <${ArrayLogo} size="md" />
          <div>
            <h1 className="text-[26px] font-bold leading-tight text-[#2A107E] md:text-[28px]">
              ${PLAN_PAGE.title}
            </h1>
            ${isDefine
              ? html`
                  <p className=${unifiedSubClass}>
                    ${PLAN_PAGE.subtitle}
                  </p>
                `
              : stage === "clarify-1" || stage === "clarify-2"
                ? html`
                    <p className=${unifiedCopyClass}>
                      ${stage === "clarify-1" ? clarifyTitle1 : clarifyTitle2}
                    </p>
                    <p className=${unifiedSubClass}>
                      ${stage === "clarify-1" ? clarifySub1 : clarifySub2}
                    </p>
                  `
                : stage === "revise"
                  ? html`
                ...
                : stage === "key-pieces" || stage === "key-pieces-loading"
                  ? html`
                      ...
                      <p className="mt-2 text-[17px] font-semibold leading-tight text-[#2A107E]">
                        ${keyPiecesSub}
                      </p>
                    `
                : stage === "the-plan"
                  ? html`
                      <p className="mt-2 text-[17px] font-semibold leading-tight text-[#21C9A2]">
                        ${thePlanReady}
                      </p>
                      <p className="mt-2 text-[17px] font-semibold leading-tight text-[#2A107E]">
                        ${thePlanSub}
